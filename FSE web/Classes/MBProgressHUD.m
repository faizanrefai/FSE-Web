...
- (void)drawRect:(CGRect)rect {
	...
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
		boxRect = CGRectMake(((allRect.size.width - self.width) / 1.65) + self.xOffset,((allRect.size.height - self.height) / 1.94) + self.yOffset, self.width/4, self.height/2.2);
	}else {
		boxRect = CGRectMake(((allRect.size.width - self.width) / 1.08) + self.xOffset,((allRect.size.height - self.height) / 1.86) + self.yOffset, self.width/4, self.height/2.2);
	}
